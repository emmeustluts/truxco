const { Output: CustomerOutstandingReportOutput } = require('../../dtos/CustomerOutstandingReport.dto');

const request = require('supertest');

describe('GET https://api.beebus.live/reports/customer-outstanding-report?lang=en', () => {
  it('should return status 200 and match expected structure', async () => {
    const res = await request('https://api.beebus.live')
      .get('/reports/customer-outstanding-report?lang=en')
    ...
    expect(res.body).toMatchObject(CustomerOutstandingReportOutput);
  });
});